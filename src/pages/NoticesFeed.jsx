...
export default function NoticesFeed() {
  ...
  //Searchable notices filtering and logic
      ...
      >
        <div className="section-header">
          <div className="section-title-group">
            <h1 className="section-title">📋 Campus Notices</h1>
          </div>
        </div>
        ...
        <div className="controls-bar">
          ...
          {/**** Total no. of available notices ****/}
          <span className="results-count">
            {loading ? "Loading..." : `${filtered.length} notice${filtered.length !== 1 ? "s" : ""} found`}
          </span>
        </div>
      </motion.div>

      {/*** Loading spinner ***/}
      {loading && <LoadingSpinner message="Loading notices..." />}

      {/*** Error msg ***/}
      {!loading && error && <ErrorMessage message={error} onRetry={retry} />}

      {/*** Setting up the notice cards as per the search query ***/}
      {!loading && !error && (
        ...
        <>
          ...
          {/**** Setting up notice cards in different pages ****/}
          <Pagination
